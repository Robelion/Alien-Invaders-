...
class Ship(GImage):
    ...
    def __init__(self, x=(GAME_WIDTH/2)):
        super().__init__(x=x,y=SHIP_BOTTOM,width=SHIP_WIDTH,height=SHIP_HEIGHT,source='ship.png')
    ...
    def moveship(self, input):
        if input.is_key_down('right'):
            ...
            if self.y <= DEFENSE_LINE - SHIP_HEIGHT/2:
                self.y += SHIP_MOVEMENT
        elif input.is_key_down('down'):
            if self.y >= 0 + SHIP_HEIGHT/2:
                self.y -= SHIP_MOVEMENT


    def shipcollides(self, bolt):
        """
        Returns: True if the bolt was fired by an alien and collided with the ship

        Parameter bolt: the laser bolt to check
        Precondition: bolt is of class Bolt
        """
        x = bolt.x - BOLT_WIDTH/2
        y = bolt.y - BOLT_HEIGHT/2

        a = bolt.x + BOLT_WIDTH/2
        b = bolt.y - BOLT_HEIGHT/2

        if self.contains((x, y)) or self.contains((a, b)):
            return True
        else:
            return False
...
class Alien(GImage):
    ...
    them below.
        ...
        Instance attributes:
            _singlealien: A singe alien [Alien]
    ...
    def aliencollides(self, bolt):
        """
        Returns True if the bolt was fired by the ship and collided with an alien
...
class Bolt(GRectangle):
    ...
    def __init__(self, x, y):
        ...
        """
        super().__init__(x = x, y = y, width = BOLT_WIDTH, height = BOLT_HEIGHT, fillcolor = 'black', velocity = BOLT_SPEED, linewidth = 10)
